...
每次文章生成完成后，**必须立即**做以下两件事，不能跳过：

1. **告知用户输出路径**，例如：
   > 文章已生成，保存在：`outputs/<YYYYMMDD>-<topic>/`
   > 目录结构：
   > ```
   > outputs/<YYYYMMDD>-<topic>/
   >   kb_raw/          ← 本次任务的知识库文档
   >   tasks/           ← 本次任务的 request.json
   >   platforms/
   >     zhihu.md       ← 知乎版本
   >     juejin.md      ← 掘金版本
   >     csdn.md        ← CSDN 版本
   >     xiaohongshu.md ← 小红书版本
   >     wechat_official.md ← 微信公众号版本
   >     toutiao.md     ← 头条版本
   >     weibo.md       ← 微博版本
   >   article.md       ← 完整主稿
   >   audit_report.json
...
2. **主动询问用户**：
   > 你想先看哪个平台版本的内容？我可以把全文发给你确认，满意后再发布。

3. **用户指定后，把对应文件的完整内容输出到对话中**，让用户能直接阅读和反馈。

4. **用户确认内容没问题后**，再执行发布操作。

> ⚠️ 不允许在用户未看到文章内容的情况下直接发布。
...
```bash
# 设置时间戳和主题（根据实际情况替换）
TASK_DIR="outputs/$(date +%Y%m%d)-<topic>"
...
for platform in juejin zhihu csdn; do
  wechatsync sync outputs/$(jq -r .task_id data/tasks/demo-request.json)/platforms/${platform}.md -p ${platform}
done
```
