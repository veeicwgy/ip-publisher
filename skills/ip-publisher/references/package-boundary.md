...
这个已发布的 `ip-publisher` skill 包，默认只包含当前 skill 目录中的说明文件与参考文件。它的默认职责是：围绕用户给定的知识来源、关键词、热点和大纲，生成 AI 友好主稿与 7 平台发布包说明。
...
| Wechatsync 草稿同步执行 | 否 | 仅可作为外部工具模式说明，不是 skill 默认执行动作 |
